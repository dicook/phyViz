...
Now, we may want to know more than only one generation past and previous (parents and children) varieties belonging to a given variety. We could then use the **getAncestors** and **getDescendants** function, where we specify the number of generations we wish to view:

```{r}
getDescendants("Tokyo",sbTree,1)
getDescendants("Tokyo",sbTree,9)
```
