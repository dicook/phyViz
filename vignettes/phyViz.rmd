...
We can confirm that the returned *ig* object is of class *igraph*.

```{r}
class(ig)
```

We can also verify that it is unweighted (UNW), and contains 230 nodes and 340 edges. Also, there are only two attributes in the ig object, the name of the vertices and the weight of the edges (which is the default value of one).

```{r}
ig
```

Moreover, we can confirm the node and edge counts with phyViz functions:
...
And also obtain the list of nodes and edges should we chose to manually examine:
