...
We can add the other data we have about each of the nodes in mygraph in several ways. The tree dataset contains informatino about each child; to add that data as is to the tree, we can use the following command: 

```{r}
mygraph <- processTreeGraph(tree, vertexinfo=c("year", "yield", "year.imputed", "min.repro.year"))
```
...
```{r}
library(plyr)
nodes <- unique(tree[,1:5])

# get a data frame of all parents whose parents are not known (i.e. parents who are not listed as children as well)
extra.nodes <- unique(data.frame(child=tree$parent[!tree$parent%in%tree$child & !is.na(tree$parent)], stringsAsFactors=FALSE))
...
mygraph <- processTreeGraph(tree, vertexinfo=nodes)
```

The mygraph object is used in many of the other functions included with this package. 
...
```{r}
getDegree("Tokyo", "Ogden", mygraph)
getDegree("Tokyo", "Holladay", mygraph)
```
...
```{r}
getPath("Brim","Bedford", mygraph, isDirected=FALSE)
```

To compute paths where direction matters, we must have a directed graph
```{r}
dirgraph <- processTreeGraph(tree, vertexinfo=nodes, isDirected = TRUE)
...
Now that we have a **path** object that consists of two lists (the variety names and years), we can plot the relationship between the two using generatePathPlot(path)

```{r}
(plotPathDF <- generatePathPlot(path))
```
...
(plotPathImage <- generatePathPlot(path))
plotPathImage
```

Visualization of Pathway between Two Vertices Superimposed on Tree:
------------------------------------------------------
If you are curious to see the above-demonstrated shortest pathway between two vertices of interest, only now superimposed over all the varieties and edges in the whole tree, **phyViz** has a function (**generateTotalPlot**) that can achieve just that. 
...
spreadTotalDF <- buildSpreadTotalDF(mygraph, binVector)
head(spreadTotalDF)
```

Next, the **buildMinusPathDF** function takes the **mygraph** object and the **path** object (from the **getPath** function) as inputs. From these objects, it creates a data frame object of the label, x, and y values of all nodes in the tree. However, the data frame object does not include the labels of the path varieties, as they will be treated differently.

```{r}
plotMinusPathDF <- buildMinusPathDF(path, mygraph)
head(plotMinusPathDF)
```

Third, the **buildEdgeTotalDF** function takes the **mygraph** object and creates a data frame object of the edges between all parent-child relationships in the graph.

```{r}
edgeTotalDF <- buildEdgeTotalDF(mygraph)
head(edgeTotalDF)
```

Fourth, the **buildPlotTotalDF** function takes the **mygraph** object and the **path** object to create a data frame object of the text label positions for the varieties in the path, as well as the edges only in the varieties in the path.

```{r}
plotTotalDF <- buildPlotTotalDF(path, mygraph)
head(plotTotalDF)
```

Now that we have generated the data frames needed to construct this type of visual object, we use them as input parameters to the **generateTotalPlot** function. 
...
```{r}
plotTotalImage <- generateTotalPlot(path=path, mygraph=mygraph)
plotTotalImage
```
...
```{r}
(plotTotalImage <- generateTotalPlot(path=path, mygraph=mygraph, binVector=1:12))
```
...
```{r}
(plotTotalImage <- generateTotalPlot(path=path, mygraph=mygraph, binVector=1:2))
```
